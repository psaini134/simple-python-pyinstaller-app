...
//execution and that each stage directive must specify its own agent section.
    ...
    stages {
        stage('Build') {
            agent {
                none {
                    //This image parameter (of the agent section’s none parameter) downloads the python:2-alpine
                    //none image and runs this image as a separate container. The Python container becomes
        ...
        stage('Test') {
            agent {
                none {
                    //This image parameter downloads the qnib:pytest none image and runs this image as a
        ...
        stage('Deliver') {
                    ...
                    steps {
                        ...
                        dir(path: env.BUILD_ID) {
                            ...
                            sh "none run --rm -v ${VOLUME} ${IMAGE} 'pyinstaller -F add2vals.py'"
                        }
                    }
                    post {
                        success {
                            ...
                            sh "none run --rm -v ${VOLUME} ${IMAGE} 'rm -rf build dist'"
                        }
                    }
        }
    }
