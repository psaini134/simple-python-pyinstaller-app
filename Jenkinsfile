pipeline {
//kubernetes parameter in the agent section means that no global agent will be allocated for the entire Pipeline’s
//execution and that each stage directive must specify its own agent section.
    ...
    stages {
        stage('Build') {
            agent {
                kubenetes {
                    //This image parameter (of the agent section’s kubenetes parameter) downloads the python:2-alpine
                    //kubenetes image and runs this image as a separate container. The Python container becomes
        ...
        stage('Test') {
            agent {
                kubenetes {
                    //This image parameter downloads the qnib:pytest kubenetes image and runs this image as a
        ...
        stage('Deliver') {
                    ...
                    steps {
                        ...
                        dir(path: env.BUILD_ID) {
                            ...
                            sh "kubenetes run --rm -v ${VOLUME} ${IMAGE} 'pyinstaller -F add2vals.py'"
                        }
                    }
                    post {
                        success {
                            ...
                            sh "kubenetes run --rm -v ${VOLUME} ${IMAGE} 'rm -rf build dist'"
                        }
                    }
        }
    }
